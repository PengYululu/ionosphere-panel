...
| `buildPeriodCheckLinePanel(snap, k, layout, bgVar)` (line 1078) | Step 2.2's right panel, top half: the background field sampled at the parcel's own position per traced step (`sampleBackgroundAlongTrajectory`), with dashed gridlines. The y-axis auto-scales to the sampled values but is always anchored at 0 and rounded to nice tick values (mirroring the notebook's `axhline(0)`, which participates in matplotlib's own autoscale) — *not* the background color scale's own (usually much wider) range, so real fluctuations read as clear wiggles. Shares `layout`'s x-axis pixel positions with `buildContourSvg` so it lines up with the contour drawn directly below it — together a JS port of `plot_background_and_ne_along_trace()` |
