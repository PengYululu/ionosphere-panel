...
| `buildPeriodCheckLinePanel(snap, k, layout, bgVar)` (line 1078) | Step 2.2's right panel, top half: the background field sampled at the parcel's own position per traced step (`sampleBackgroundAlongTrajectory`). The y-axis uses `bgVar.vmin`/`bgVar.vmax` directly (the same fixed range as the background color scale, including any user override) rather than auto-scaling to the sampled values, so the line's height is directly comparable to the color legend. Shares `layout`'s x-axis pixel positions with `buildContourSvg` so it lines up with the contour drawn directly below it — together a JS port of `plot_background_and_ne_along_trace()` |
